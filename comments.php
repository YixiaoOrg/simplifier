...
<li id="li-<?php $comments->theId(); ?>" class="<?php echo $clevel;?>">
    <div id="<?php $comments->theId(); ?>">
        <?php
        $a = 'https://cravatar.cn/avatar/' . md5(strtolower($comments->mail)) . '?s=80&r=X&d=';
        ?>
        <img class="avatar" src="<?php echo $a ?>" alt="<?php echo $comments->author; ?>" />
        <div class="cp">
            <?php $comments->content(); ?>
            <div class="cm">
                <span class="ca"><?php echo $author ?></span>&nbsp;<?php echo $cby;?>&nbsp;<?php $comments->date(); ?><span class="cr"><?php $comments->reply(); ?></span>
            </div>
        </div>
    </div>
...
<?php else: ?>
    <?php if($this->remember('author',true) != "" && $this->remember('mail',true) != "") : ?>
        <span>欢迎【<?php $this->remember('author'); ?>】的归来 | <small style="cursor: pointer;" onclick = "tg_c('ainfo','hinfo');"> 编辑资料</small>
</span>
